...
-------------------------------------------------
    ...
    library(ggplot2)
    total.steps <- tapply(A$steps, A$date, FUN=sum, na.rm=TRUE)
    qplot(total.steps,binwidth=1000,xlab="ttl # of steps taken each day")
...
-------------------------------------------
    ...
    avg <- aggregate(x=list(steps=A$steps),     
...
![](PA1_template_files/figure-markdown_strict/unnamed-chunk-2-1.png) On
average across all the days in the dataset, the 5-minute interval
...
weekends? First, let's find the day of the week for each measurement in
the dataset. In this part, we use the dataset with the filled-in values.
